...
Scenario: Fork a git repo
    When I execute git-obs with args "fork pool test-GitPkgA"
...
Scenario: Fork a git repo twice under different names
    ...
    Then the exit code is 0
     ...
     And stderr is
         ...
         * Fork created: Admin/test-GitPkgA
        """
    When I execute git-obs with args "fork pool test-GitPkgA --new-repo-name=new-package"
    Then the exit code is 0
     And stdout is
        """
        """
...
Scenario: Fork a git repo from pool and fork someone else's fork of the same repo
    ...
    Then the exit code is 0
     ...
     And stderr is
         ...
         * Fork created: Admin/test-GitPkgA
        """
    When I execute git-obs with args "fork -G alice pool test-GitPkgA --new-repo-name=test-GitPkgA-alice"
    Then the exit code is 0
     And stdout is
        """
        """
    ...
    When I execute git-obs with args "fork Alice test-GitPkgA-alice"
    Then the exit code is 0
     And stdout is
        """
        """
     And stderr is
        ...
        """
    When I execute git-obs with args "clone Admin test-GitPkgA-alice --no-ssh-strict-host-key-checking"
    Then the exit code is 0
