    ...
    def run(self, args):
        ...
        try:
            ...
            try:
                ...
                commits = git._run_git(["rev-list", f"{lfs_remote}/{source_branch}", f"^{target_branch}"]).splitlines()
            ...
            try:
                git._run_git(["merge", "-X", "theirs", "--no-edit", source_ref])
            except Exception as e:
            ...
            # Create PR
            title = f"Forward {source_branch} to {target_branch}"
            description = f"Automated forward of {source_branch} to {target_branch} using git-obs."
