...
class ForkObsPackageCommand(osc.commandline_git.GitObsCommand):
    """
    Fork an OBS package that is managed in Gitea
    """
    ...
    def run(self, args):
        ...
        self.gitea_login = self.gitea_conf.get_login_by_url_user(url=url, user=osc_conf.get_apiurl_usr(args.apiurl))
        ...
        parent_branch_data = gitea_api.Branch.get(self.gitea_conn, owner, repo, fork_branch).json()

        try:
            repo_data = gitea_api.Fork.create(self.gitea_conn, owner, repo, new_repo_name=args.new_repo_name).json()
        ...
        fork_branch_data = gitea_api.Branch.get(self.gitea_conn, fork_owner, fork_repo, fork_branch).json()
