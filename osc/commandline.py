    ...
    def do_repairlink(self, subcmd, opts, *args):
        ...
        * Pull conflicting changes from one project into another one:
        ...
        for name in sorted(entries.keys()):
            ...
            if md5_new != '':
                self.download(name, md5_new, dir_new, store_sources_get_path(destdir, name))
            if md5_old == md5_new:
                if md5_oldpatched == '':
            ...
            if md5_old == md5_oldpatched:
                ...
                shutil.copy2(store_sources_get_path(destdir, name), os.path.join(destdir, name))
                continue
            ...
            if md5_new != '':
                shutil.copy2(store_sources_get_path(destdir, name), os.path.join(destdir, name + '.new'))
            else:
